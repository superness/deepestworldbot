...
const workerCode = `
    ...
    function getMonsterBattleScore(monster, c, useFullHp = false) {
        ...
        return hpUse * getMonsterDmg(monster) * (isEle ? 1.6 : 1) * (monster.md.toLowerCase().includes('alarm') ? 3 : 1) * (monster.md.toLowerCase().includes('spiked') ? 1.3 : 1)
    }
    ...
    function getSpotInfo(x, y, monsters, nonTraversableEntities, c, gearTesting, optimalMonsterRange, optimalMonsterRangeBuffer, targetZoneLevel, targetId, nearMonsterUnsafeRadius) {
        ...
        if (spotType != "obstructed" && spotType != "dangerous") {
            for (let monster of nearMonsters) {
                ...
                if (dist < optimalMonsterRange + optimalMonsterRangeBuffer && !gearTesting && isValidTarget(monster, nonTraversableEntities, c, monsters, targetZoneLevel, nearMonsterUnsafeRadius)) {
                    let delta = 0
        ...
        function* yieldVisionGridUpdatesOnOldSpots() {
            while (true) {
                ...
                for (let spot of visionGridEx) {
                    ...
                    let spotInfo = getSpotInfo(x, y, e.data.monsters, e.data.nonTraversableEntities, e.data.c, e.data.gearTesting, e.data.optimalMonsterRange, e.data.optimalMonsterRangeBuffer, e.data.targetZoneLevel, targetId, e.data.nearMonsterUnsafeRadius)
...
setInterval(function () {
    ...
    {
        statusMessage = `GEAR TESTING: ${gearTesting} COMBAT: ${dw.c.combat}`
        return
    }
    ...
    if ((!target || target.hp == target.hpMax) && monsterTargettingMe && target != monsterTargettingMe) {
        ...
        combatText = "SETTING TARGET TO NEARBY MONSTER THAT IS TARGETTING ME " + target.md + getMonsterBattleScore(target)
    ...
    statusMessage = `Target this entity: ${JSON.stringify({id:target.id, md:target.md, t:target.targetId, l: target.level, tr: target.terrain, h: target.bad, s: getMonsterBattleScore(target).toFixed()})}`
    ...
    if (dw.c.mp < (skillUse?.cost ?? 0) || !dw.isSkillReady(skillUse.md) || dw.distance(target, dw.c) > optimalMonsterRange) {
        return
    }
    ...
    if(dw.isSkillReady(skillUse.md))
    {
...
setInterval(() => {
    ...
    if(dw.c.combat) return

    let curZoneLevel = Math.floor(dw.distance({x:0,y:0}, dw.c) / 16)+1
    let curSpawnLevel = Math.floor(dw.distance({x:0,y:0}, dw.c.respawn) / 16)+1

    if(curZoneLevel > curSpawnLevel && dw.c.l == 0 && ((dw.c.level - 6) >= curZoneLevel))
    {
        console.log('setting respawn because it is a higher level zone')
        dw.emit('setRespawn')
    }
}, 500)
...
setInterval(() => {
    if(combatText != null) {
        let newText = { text: combatText, x: dw.c.x, y: dw.c.y, target: dw.c, life: 2, maxLife: 2 }
        floatingText.push(newText)
        combatText = null
    }
}, 1000)
...
let prevMission = dw.c.mission;
setInterval(function () {
    if (!dw.c.mission && prevMission) {
        // TODO: Mission complete, go home
    }
    prevMission = dw.c.mission;
}, 500);
...
// setInterval(function () {
//     let missionTable = dw.findEntities((e) => e.md.includes("missionTable")).shift();
//     if (!missionTable)
//         return;
//     if (!dw.c.mission)
//         return;
//     if (dw.distance(missionTable, dw.c) > 2) {
//         moveToSpot = missionTable;
//     } else {
//         if (forDisenchantList.length == 0) {
//             dw.emit("enterMission");
//         }
//     }
// }, 5e3);
...
let deleteItemTypes = ["taunt", "dash", "lifeshield", "heal", "charge"]
...
async function tryoutFoundGear() {
    ...
    let inARow = 0;
    while (gearTesting) {
        let currentScore = getMyBattleScore(true);
        await tryGearLoop();
        let newScore = getMyBattleScore(true);
...
async function tryoutSkill(i) {
    let bestSkill = getBestSkill(0);
    let currentBattleScore = getMyBattleScore(true);
    console.log("trying skill in index", i);
    dw.moveItem("bag", i, "skillBag", skillTestSlot);
    await sleep(gearChangeCD);
    bestSkill = getBestSkill(0);
    let newBattleScore = getMyBattleScore(true);
...
async function tryoutItem(i, item) {
    ...
    if (item.md.toLowerCase().includes("ring")) {
        currentBattleScore = getMyBattleScore(true)
        dw.emit("equip", { slot: "ring2", i })
        await sleep(gearChangeCD)
        newBattleScore = getMyBattleScore(true)
        console.log(currentBattleScore, " -> ", newBattleScore)
        if (newBattleScore > currentBattleScore)
            return;
        dw.emit("equip", { slot: "ring2", i })
...
setTimeout(tryoutFoundGear, 500)


let farmTrees = false;

// Add a button to the window to toggle tree farming status
addTreeFarmingButton()


function addTreeFarmingButton() {

    for(var prevBtn of window.top.document.getElementsByClassName('toggle-treefarm'))
    {
        prevBtn.remove()
    }
    ...
    newi.onclick = () => { 
        ...
        dw.set(`${dw.c.name}_farmTrees`, farmTrees)
    }

    var newMenuButton = window.top.document.createElement('div')
    newMenuButton.className = 'ui-btn px-1 toggle-treefarm'

    newMenuButton.appendChild(newi)

    var menuButtons = window.top.document.getElementById('menuButtons')
    menuButtons.appendChild(newMenuButton)
}

setInterval(function () {
    farmTrees = dw.get(`${dw.c.name}_farmTrees`) ?? farmTrees
    if (!farmTrees)
        return;
    let target = dw.findEntities((entity) => entity.id === dw.targetId).shift()
    let tree = dw.findEntities((e) => e.md == "maple" || e.md.startsWith("cactus")).filter((e) => hasLineOfSight(e, dw.c, getNonTraversableEntities())).sort((a, b) => a.x + a.y - (b.x + b.y)).shift();
    if (target && (target.md == "maple" || target.md == "cactus")) {
        tree = target
    }
    if (!tree) {
        if (dw.distance(moveToSpot, dw.c) < 0.5) {
            moveToSpot = getGoodSpots(50).filter((s) => dw.distance(s, dw.c) > searchOffset).shift()
        }
        console.log("no tree", tree)
        return;
    }
    ...
    if (distance > 0.7) {
        ...
        if (target && target != tree)
            return
        console.log("chop")
        if (tree.md == "maple" || tree.md.startsWith("cactus")) {
            if (dw.isSkillReady("chop")) {
                dw.emit("chop", tree)
            }
        } else {
            if (dw.isSkillReady("mine")) {
                dw.emit("mine", tree)
            }
        }
    }
...
setInterval(() => {
    dw.log(dw.findEntities(e => e.targetId == dw.c.id).map(e => e.md)) 

    let target = dw.findEntities((entity) => entity.id === dw.targetId).shift()
    //statusMessage = `Target: ${JSON.stringify(target)}`

    statusMessage2 = `g:${gearTesting} c:${dw.c.combat} i:  ${dw.c.id} MP: ${dw.c.mp} t: ${dw.targetId}, bs:${target ? getMyBattleScore(target).toFixed() : -1} r:${getBestSkill(1000)?.range ?? -1} ${new Date().toTimeString()}`
}, 1000)
...
//     let x = ctx.canvas.width / 2 - 300
//     let y = ctx.canvas.height / 2
    
//     ctx.fillStyle = 'yellow'

//     drawText(ctx, 14, statusMessage, x, y)
//     drawText(ctx, 14, statusMessage2, x, y + 20)
//     drawText(ctx, 14, new Date().toTimeString(), x, y + 40)
// })
...
dw.on('drawEnd', (ctx) => {
    ...
    const border = 2
    ctx.beginPath()
    ctx.rect(ctx.canvas.width - xp.length * 2 - 16 - border, 64 - border, xp.length * 2 + border * 2, 64 + border * 2)
    ctx.stroke()
    ctx.fill()
    ...
    ctx.beginPath()
    ctx.rect(ctx.canvas.width - 16 - 2 * i, 128 - value, 2, value)
    ctx.fill()
    }
    ...
    ctx.strokeText(text, ctx.canvas.width - 16, 150)
    ctx.fillText(text, ctx.canvas.width - 16, 150)

    if (xpPerHour > 0) {
    text = `${formatRemainingDuration(Math.ceil((max - dw.c.xp) / xpPerHour * 3600))} to level up`
    ctx.strokeText(text, ctx.canvas.width - 16, 174)
    ctx.fillText(text, ctx.canvas.width - 16, 174)
    }
})
  ...
  for (const entry of entries) {
    ...
    requests.push(fetch(
      `${baseUrl}/log/loot`,
      {
        ...
        body: JSON.stringify({
          md: entry.item.md,
          qual: entry.item.qual,
          mods: entry.item.mods,
          cmps: entry.item.cmps,
        })
      }
    ))
  }
...
dw.on('drawEnd', (ctx) => {
    ...
    const y = 48
    const text = `🎯${targetZoneLevel.toLocaleString()}   🌀${dw.getZoneLevel(dw.c.respawn.l ?? 0, dw.c.respawn?.x ?? 0, dw.c.respawn.y ?? 0)}`
    ctx.strokeText(text, x, y)
    ctx.fillText(text, x, y)
  })
...
setInterval(() => {
    if(getBestSkill(0).fx.blink)
    {
        nearMonsterUnsafeRadius = 2
    }
    else
    {
        nearMonsterUnsafeRadius = 2
